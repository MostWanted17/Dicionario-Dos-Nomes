...
ARQUIVO_JSON = "nomes_masculinos_todas_paginas.json"
...
def detectar_genero(nome):
    ...
    if nome_formatado == "mikhael":
        url_nome = "https://www.dicionariodenomesproprios.com.br/mikael/"
    elif nome_formatado == "zebulom":
        url_nome = "https://www.dicionariodenomesproprios.com.br/zebulon/"
    else:
...
def obter_dados_nome(nome):
    """Obtém os dados do nome se for masculino, senão ignora."""
    if detectar_genero(nome) == "feminino":
        print(f"⏭️ {nome} - Gênero feminino detectado. Pulando...")
        return None
...
def coletar_nomes_relacionados(nome):
    """Coleta nomes relacionados, mas ignora femininos."""
    nome_formatado = remover_acentos(nome.lower()).replace(" ", "-")
    ...
    if ul_relacionados:
        for li in ul_relacionados.find_all("li"):
            ...
            if a:
                ...
                # Verifica se o nome é masculino antes de coletar
                if detectar_genero(nome_relacionado) == "feminino":
                    print(f"⏭️ Nome relacionado {nome_relacionado} é feminino. Pulando...")
                    continue
...
def coletar_top_nomes():
    print("\nPegando a Lista dos Nomes Populares")
    url_nomes_top = "https://www.dicionariodenomesproprios.com.br/nomes-masculinos/"
    soup_nomes_top = obter_soup(url_nomes_top)
    lista_top = soup_nomes_top.find("ol", class_="gender-top--list male")
...
def coletar_top_nomes_por_letra():
    print("\nColetando Nomes Populares por letra:")
    for letra in string.ascii_lowercase:
        url_letra = f"https://www.dicionariodenomesproprios.com.br/nomes-masculinos/{letra}/"
        print(f"Processando letra: {letra.upper()}")
        try:
            soup_letra = obter_soup(url_letra)
            lista_letra = soup_letra.find("ol", class_="gender-top--list male")
...
for pagina in range(205, 205):  # Ajuste o range conforme necessário
    ...
    for nome_span in soup.find_all("span", class_="list-wide--name full-w"):
        ...
        if nome not in nomes_existentes:
            ...
            print(f"⏭️ {nome} já coletado. Pulando...")
